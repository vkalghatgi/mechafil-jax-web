...
Minting is broken in to relevant components as shown below
...
Propose adjusting the initial consensus pledge calculation such that it has a "simple" consensus pledge $P_{C,S}$ and "baseline" consensus pledge component $P_{C,B}$, determined by some weighting paramaterized by $\gamma'$, analagous to the minting model. This can be implemented with a number of weighting schemes that interpolate differently between the regions $0 \le \gamma' \le 1$. Some of these weighting schemas are specified below: 

### Arithmetic Weighting 

This is most analagous to the current minting model and $P_{C}$ is calcualted as per below: 
